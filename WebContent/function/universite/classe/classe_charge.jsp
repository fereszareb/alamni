...
try {
	...
	String sql = "SELECT classe ,f.id ,count(*) as nb FROM `etudiant`, classe f WHERE( id_classe = f.id ) GROUP by id_classe";
...
try {
	connection = DriverManager.getConnection("jdbc:mysql://localhost:3306/alamni", "root", "");
	statement = connection.createStatement();
	String sql = "SELECT * FROM `classe` where ( id not in (SELECT DISTINCt(id_classe) from etudiant ))";
	resultSet = statement.executeQuery(sql);
	while (resultSet.next()) {
%>
